const { getGitHubPullRequest } = require("./../utils/gitHubUtil");
const {
...
async function convertDefaultHash(
  owner,
  repo,
  defaultHash,
  write,
  ...
  try {
    ...
    if (write) {
      ...
      const resPostTsrcID = await postCreateIssue(
        `${owner}/${repo}`,
        defaultHash,
        head
      );
...
const root = {
  ...
  getPRvoteYesTotals: async function (args) {
    const convertedHashes = await convertDefaultHash(
      args.owner,
      args.repo,
      args.defaultHash,
      false,
  ...
  getVotePowerAmount: async function (args) {
  ...
  setVote: async function (args) {
    ...
    convertedHashes = await convertDefaultHash(
      args.owner,
      args.repo,
      args.defaultHash,
      true,
